...
const JobDesc = () => {
  ...
  const handleJobDes = () => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/job/${id}`)
      .then((res) => {
        setJob(res.data.job);
        console.log(job);
      })
    ...
    <div className="flex" style={{ backgroundColor: "#FFEFEF" }}>
      <div className="job-desc">
        ...
        <div className="top-job-desc">
          <p>
          {job ? job.position : ""}
            {job.jobType ? " work from office " : " work from home "}
            job/internship at
            <br />
            {job.companyName}
          </p>
        ...
        <div className="bottom-job-desc">
          ...
          <div className="des-title flex">
            <h3 style={{ fontSize: "1.8rem" }}> {job ? job.position : ""}</h3>
            <button className="edit-job">Edit job</button>
          </div>
          <div className="place">{job ? job.location : ""}</div>
          ...
          <div className="about about-company">
            ...
            <p className="about-des"> {job ? job.aboutCompany :""}</p>
          </div>
          <div className=" about about-job">
            <h3 className="about-title">About the job/internship</h3>
            <p className="about-des">{job ? job.description : ""}</p>
          </div>
